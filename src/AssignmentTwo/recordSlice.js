  ...
  reducers: {
        ...
        value: state.value.map((record) => {
              for (let i = 0; i < action.payload.length; i++) {
                  if(action.payload[i] === record.id) {
                      let tempRecord = {...record, "display":true }
                      return tempRecord
                  }
              }
              return record
    ...
    showSelectMovieForEdit: function (state, action) {
      return {
        ...
        value: state.value.map((item, index) =>
          item.id !== action.payload
            ? { ...item, displayOneMovie: false }
            : { ...item, displayOneMovie: true }
        ),
      };
    },
    ...
    showSelectedPinnedMovie: function (state, action) {
        return {
          ...
          value: state.value.map((record) => {
                for (let i = 0; i < action.payload.length; i++) {
                    if(action.payload[i] === record.id) {
                        let tempRecord = {...record, "selecttobepinned":true }
                        return tempRecord
                    }
                }
                return {...record,  "selecttobepinned":false }
            })
        };
    },
      ...
      return {
        ...
        value: state.value.map((record) => {
              if(action.payload.id === record.id) {
                  return {...record, "description": action.payload.description}
              }
              return record
          })
      }
    ...
    saveMovieChangeName: function(state,action){
      return {
        ...
        value: state.value.map((record) => {
              if(action.payload.id === record.id) {
                  return {...record, "movie name":action.payload["movie name"]}
              }
              return record
          })
      }
    ...
    saveMovieChangeGenre: function(state,action) {
      return {
        ...
        value: state.value.map((record) => {
              if(action.payload.id === record.id) {
                  return {...record, "genre":action.payload.genre }
              }
              return record
          })
      }
    ...
    saveMovieChangeLeadActor: function (state, action) {
      return {
        ...
        value: state.value.map((record) => {
              if(action.payload.id === record.id) {
                  return {...record, "lead actor":action.payload["lead actor"].split(",") }
              }
              return record
          })
      }
    ...
    saveMovieChangeRating: function (state, action) {
      return {
        ...
        value: state.value.map((record) => {
              if(action.payload.id === record.id) {
                  return {...record, "rating":action.payload.rating }
              }
              return record
          })
      }
    ...
    saveMovieChangeLanguage: function (state, action) {
      return {
        ...
        value: state.value.map((record) => {
              if(action.payload.id === record.id) {
                  return {...record, "language":action.payload.language }
              }
              return record
          })
      }
